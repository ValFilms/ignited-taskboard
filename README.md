...
1. Create a dedicated Supabase project. Run `supabase/schema.sql`. Storage bucket `raw-footage` must remain private with no public read policies. Check your plan's maximum upload size; the app caps individual files at 1 GB, but plan limits may be lower.
