...
1. Google Form submission creates an Onboarding client with all checklist items incomplete. Required payload fields: stable submission ID, business name, person, email, location; optional contact values may be empty strings. The connected sheet maps **Business owner name** to the client's contact name (`person`), accepting differences in heading capitalization and whitespace. It prefers that nonblank value over the legacy **Person name** field. Blank older responses preserve an existing contact name on sync. The business owner is separate from the agency teammate assigned as client owner. Form receipt is separate from onboarding completion.
