...
export default function PasswordSettings({ configured, save, onboarding = false, onDemoComplete }: {
  configured: boolean;
  ...
  async function submit(event: React.FormEvent) {
    ...
    try {
      ...
      if (!configured) { setCurrent(""); setNew(""); setConfirm(""); setShow(false); setMessage("Demo preview only. No password was changed. Sign in to the live workspace to save your own password."); onDemoComplete?.(); return; }
  ...
  return <section className="panel">
    <div className="panel-heading"><h2><KeyRound size={18} /> {onboarding ? "Make your account yours" : "Change password"}</h2></div>
    <form className="settings-body" onSubmit={event => void submit(event)}>
      ...
      <label>New password<input type={show ? "text" : "password"} autoComplete="new-password" required maxLength={256} aria-describedby={helpId} disabled={busy} value={newPassword} onChange={e => setNew(e.target.value)} /></label>
      <p id={helpId} className="muted">Use at least 8 characters. A longer, unique passphrase works well.</p>
      ...
      <div className="password-actions">
        ...
        <button type="submit" className="primary" disabled={busy}>{busy ? "Updating…" : onboarding ? "Save password & finish" : "Update password"}</button>
      </div>
