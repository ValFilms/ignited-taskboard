...
import PhoneSetupGuide from "./phone-setup-guide";

export default function PushSettings({ configured, api, member }: { configured: boolean; member: Member; api: (path: string, body: unknown) => Promise<any> }) {
  ...
  useEffect(() => {
    let active = true;
    setReady(false); setEnabled(false); setPublicKey(""); setMessage("");
    ...
    return () => { active = false; };
    // Check on panel open or explicit refresh; API uses the current session.
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [configured, member.id, check]);
  ...
  return <section className="panel"><div className="panel-heading"><h2><Bell size={18} /> Push notifications</h2></div>
    <div className="settings-body">
      <p>Get notified about messages, mentions, assignments, completed tasks, approvals and deadlines, even when the app is closed.</p>
      ...
      <div className="push-actions">
        {enabled ? <><button className="primary" disabled={busy || !ready} onClick={() => void deviceAction("test")}>Send test notification</button><button className="secondary" disabled={busy || !ready} onClick={() => void deviceAction("unsubscribe")}>Disable on this device</button></> :
          <button className="primary" disabled={busy || !ready || !configured || !supported || !publicKey || iosInstall || permission === "denied"} onClick={() => void enable()}>{busy ? "Enabling…" : "Enable push notifications"}</button>}
        <button className="secondary" disabled={busy || !ready} onClick={() => setCheck(value => value + 1)}>Check device status</button>
      </div>
      {message && <p className="push-help" role="status">{message}</p>}
      <PhoneSetupGuide member={member} />
