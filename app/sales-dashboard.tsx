...
export default function SalesDashboard({configured,api}:{configured:boolean;api:(path:string,body:unknown)=>Promise<SalesData>}) {
  ...
  return <section className="sales-dashboard" aria-label="Sales dashboard">
    ...
    {data&&data.mode!=="disconnected"&&summary&&<>
      ...
      <div className="sales-outcomes"><span><CalendarDays size={16}/>Appointment outcomes</span><b>{data.complete.events?summary.showed:"—"} attended</b><b>{data.complete.events?summary.noShow:"—"} no-show</b><b>{data.complete.events?summary.cancelled:"—"} cancelled</b><span>In the selected date range</span></div>
